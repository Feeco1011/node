...
router.post('/', async (req, res, next) => {
  ...
  // 有沒有重覆的email
  const [rows] = await db.query('SELECT * FROM m_data WHERE m_email = ?', [
    newMember.email,
  ])

  if (rows.length > 0) {
    return res.json({ status: 'error', message: '有重覆的電子信箱' })
  }
  ...
  const [rows2] = await db.query(
    "INSERT INTO `m_data`(`m_name`,`m_password`,`m_email`,`m_phone`,`m_sex`,`m_created_at`) VALUES(?,?,?,?,1,'2024-10-18 00:00:00')",
    // "INSERT INTO `m_data`(`m_name`,`m_password`,`m_email`,`m_phone`,`m_sex`,`m_created_at`,`m_updated_at`) VALUES(?,?,?,?,1,'2024-10-18 00:00:00','2024-10-18 00:00:00')",
    [
      newMember.name,
      passwordHash,
      newMember.email,
      newMember.mobile,
      Number(newMember.sex),
    ]
  )
...
router.post('/login', async (req, res, next) => {
  ...
  const isValid = await compareHash(loginMember.password, dbMember.m_password)
